...
const COIN_LUCK_CHANCE = 0.08;
const COIN_TYPES = Object.freeze([
  {key:'copper', name:'Медная монета', icon:'🟤', add:10, weight:60},
  {key:'silver', name:'Серебряная монета', icon:'⚪', add:20, weight:30},
  {key:'gold', name:'Золотая монета', icon:'🟡', add:30, weight:10}
]);
...
function renderCoinImpacts(fish) {
  ...
  return impacts.map(impact=>{
    ...
    return `<div class="gear-impact coin-impact"><span class="gear-icon">${impact.icon}</span><span><strong>${impact.name} +${impact.add} кг</strong> <span class="gear-arrow">→</span> <span class="gear-target">${capitalize(fish.name)}</span><small>${kg(impact.before)} + ${impact.add} кг = <b class="gear-result">${kg(impact.after)}</b></small>${limited}</span></div>`;
  }).join('');
}
...
function processCoinCatch() {
  ...
  const coin={id:uid(), type:type.key, name:type.name, icon:type.icon, add:type.add, used:false, success:false, applied:false, expired:false, historyRowId:null};
  state.coins.push(coin);
  const row=addHistory(type.name,'coin',`(Шанс 8%: следующая пойманная рыба получит +${type.add} кг)`,{coinId:coin.id});
...
function useCoin(coinId) {
  ...
  coin.used=true;
  coin.success=chance(COIN_LUCK_CHANCE);
  const row=state.history.find(item=>item.id===coin.historyRowId);
  if (coin.success) {
    state.pendingCoinBoosts.push({coinId:coin.id, rowId:coin.historyRowId, add:coin.add, name:coin.name, icon:coin.icon});
...
function makeFish(category='normal', source='Заброс', direct=true, options={}) {
  ...
  if (Array.isArray(state.pendingCoinBoosts) && state.pendingCoinBoosts.length) {
    ...
    state.pendingCoinBoosts.forEach(boost=>{
      ...
      f.coinImpacts.push({coinId:boost.coinId, name:boost.name, icon:boost.icon, add:boost.add, before, after:f.weight});
      const coin=state.coins.find(item=>item.id===boost.coinId);
